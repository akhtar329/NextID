...
if (!filters.showClosed) {
  ...
  conditions.push(eq(admissions.status, 'Closed'));
    ...
    if (!filters.showClosed && filters.level && filters.level in LEVEL_KEYWORDS) {
...
function Breadcrumbs({ filters, programTypes, citiesWithCounts, showClosed }: { 
  filters: { city?: string; level?: string; q?: string };
  ...
  const items = [
    { name: 'Home', url: '/' },
    { name: showClosed ? 'Closed Admissions' : 'Admissions', url: showClosed ? '/admissions?closed=true' : '/admissions' },
  ];

  if (filters.level) {
    const level = programTypes.find(l => l.slug === filters.level);
    if (level) {
      items.push({ name: level.name, url: `/admissions?level=${filters.level}${showClosed ? '&closed=true' : ''}` });
    }
  }
  ...
  if (filters.city) {
    ...
    if (city) {
      items.push({ name: city.name, url: `/admissions?city=${filters.city}${showClosed ? '&closed=true' : ''}` });
    }
  }

  if (filters.q) {
    items.push({ name: `Search: ${filters.q}`, url: `/admissions?q=${filters.q}${showClosed ? '&closed=true' : ''}` });
  }
...
const [openAdmissionsResult, closedAdmissionsResult, stats, citiesWithCounts, closedCitiesWithCounts] = await Promise.all([
  ...
  const uniqueUniversities = new Set(openAdmissionsResult.admissions.map(a => a.instituteName)).size;
  const totalPrograms = openAdmissionsResult.admissions.reduce((sum, ad) => sum + ad.programs.length, 0);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
          ...
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full mb-6">
              ...
              <span className="text-xs font-medium text-white">
                {showClosed ? 'Past Admissions Records' : 'Admissions Open for 2026'}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10">
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-5 text-center border border-white/20 hover:bg-white/20 transition">
                <div className="text-3xl font-bold">{stats.closedAdmissions}+</div>
                <div className="text-sm text-blue-200 mt-1">Closed Admissions</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-5 text-center border border-white/20 hover:bg-white/20 transition">
              ...
              <form action="/admissions" method="GET" className="flex gap-2">
                ...
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    placeholder={showClosed ? "Search closed admissions..." : "Search by university or program..."}
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <Breadcrumbs filters={filters} programTypes={PROGRAM_TYPES} citiesWithCounts={currentCitiesWithCounts} showClosed={showClosed} />
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    All ({showClosed ? stats.closedAdmissions : stats.totalAdmissions})
                  </Link>
              ...
              {!showClosed && filters.level && FEE_RANGES[filters.level as LevelType] && (
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h2 className="text-2xl font-bold text-gray-800">
                    {currentAdmissions.totalCount} {showClosed ? 'Closed' : 'Open'} Admissions Found
                  </h2>
                  <p className="text-sm text-gray-500 mt-1">
                    {filters.level && !showClosed && `Level: ${PROGRAM_TYPES.find(l => l.slug === filters.level)?.name}`}
                    {filters.city && ` • City: ${currentCitiesWithCounts.find(c => c.slug === filters.city)?.name}`}
                              ...
                              <div className="flex-1">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                  <div className="bg-gray-50 rounded-xl p-3">
                                    <span className="text-gray-500 text-xs">{showClosed ? 'Closed Date' : 'Last Date'}</span>
                ...
                <div className="bg-white rounded-2xl shadow-sm p-16 text-center border border-gray-100">
                  ...
                  <p className="text-gray-500 mb-6">
                    {filters.level || filters.city || filters.q 
                      ...
                      : showClosed ? 'No closed admissions found' : 'Check back soon for latest admissions'}
                  </p>
                  <Link
                    href={showClosed ? '/admissions?closed=true' : '/admissions'}
                    className="inline-block px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition"
                  >
                    {showClosed ? 'View Open Admissions' : 'View Closed Admissions'}
                  </Link>
                </div>
              )}
            </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-bold text-gray-900 mb-8">
              {showClosed ? 'About Closed Admissions in Pakistan' : 'About Admissions 2026 in Pakistan'}
            </h2>
            
            <div className="space-y-6 text-gray-700 leading-relaxed">
              <p>
                <strong className="text-gray-900">
                  {showClosed ? 'Closed Admissions Records in Pakistan' : 'Admissions 2026 in Pakistan'}
                </strong> 
                {showClosed 
                  ? ' provide valuable reference for future applicants. Review past admission schedules, requirements, and deadlines to prepare for upcoming admissions.'
                  : ` are now open in top universities and colleges across the country. 
                  ...
                  Major cities including ${currentCitiesWithCounts.slice(0, 5).map((city, i, arr) => (
                    <span key={city.slug}>
              ...
              <p>
                <strong className="text-gray-900">
                  {showClosed ? 'Past admission deadlines' : 'Last dates for admissions 2026'}
                </strong> 
                {showClosed
                  ? ' varied by university and program. Most universities closed admissions by March-April for Fall semester.'
                  : ` vary by university and program. Most universities close 
              ...
              <p>
                <strong className="text-gray-900">
                  {showClosed ? 'Historical admission fees and test requirements' : 'Admission fees and entry tests'}
                </strong> 
                {showClosed
                  ? ' varied by program level and institution. Review past records to estimate future costs.'
              ...
              <p>
                <strong className="text-gray-900">
                  {showClosed ? 'Top universities with past admissions' : 'Top universities accepting admissions 2026'}
                </strong> 
                {showClosed
                  ? ' include NUST, FAST, LUMS, University of the Punjab, and Karachi University. Check individual university pages for program-specific details.'
                  : ` include NUST, FAST, LUMS, University of the Punjab, 
            ...
            <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-4">
              {currentCitiesWithCounts.slice(0, 4).map(city => (
                <Link
                  key={city.slug}
                ...
                >
                  ...
                  <div className="text-sm text-gray-600 mt-1 group-hover:text-blue-600">
                    {showClosed ? 'Closed in' : 'in'} {city.name}
                  </div>
                </Link>
              ))}
            </div>
