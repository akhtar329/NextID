// app/sitemaps/llms.xml/route.ts
...
export async function GET() {
  ...
  try {
    const postsList = await db
      ...
      .from(posts)
      .where(
        sql`${posts.type} = 'llm' AND ${posts.status} = 'published'`
      )
      .limit(1000);
...
${postsList.map((post) => `
  <url>
    <loc>${BASE_URL}/llms/${post.slug}</loc>
    ...
    console.error("Error generating llms sitemap:", error);
